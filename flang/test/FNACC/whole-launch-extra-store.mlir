// RUN: not fir-opt \
// RUN:   --fnacc-assign-kernel-ids \
// RUN:   --fnacc-lower-to-runtime \
// RUN:   %s -o /dev/null 2>&1 | FileCheck %s

module {
  func.func @extra_store(
      %nref: !fir.ref<i32>,
      %i: !fir.ref<i32>,
      %a: !fir.ref<!fir.array<?xf32>>,
      %b: !fir.ref<!fir.array<?xf32>>,
      %c: !fir.ref<!fir.array<?xf32>>,
      %side: !fir.ref<f32>) {
    %one = arith.constant 1 : i32
    %n = fir.load %nref : !fir.ref<i32>
    %nidx = fir.convert %n : (i32) -> index
    %shape = fir.shape %nidx : (index) -> !fir.shape<1>
    %side_value = arith.constant 42.0 : f32

    fnacc.launch tile_sizes = [128] {
      fir.do_loop %iv = %one to %n step %one : i32 {
        fir.store %iv to %i : !fir.ref<i32>

        %i0 = fir.load %i : !fir.ref<i32>
        %idx0 = fir.convert %i0 : (i32) -> i64
        %ap = fir.array_coor %a(%shape) %idx0
            : (!fir.ref<!fir.array<?xf32>>, !fir.shape<1>, i64)
              -> !fir.ref<f32>
        %av = fir.load %ap : !fir.ref<f32>

        %i1 = fir.load %i : !fir.ref<i32>
        %idx1 = fir.convert %i1 : (i32) -> i64
        %bp = fir.array_coor %b(%shape) %idx1
            : (!fir.ref<!fir.array<?xf32>>, !fir.shape<1>, i64)
              -> !fir.ref<f32>
        %bv = fir.load %bp : !fir.ref<f32>

        %sum = arith.addf %av, %bv : f32

        %i2 = fir.load %i : !fir.ref<i32>
        %idx2 = fir.convert %i2 : (i32) -> i64
        %cp = fir.array_coor %c(%shape) %idx2
            : (!fir.ref<!fir.array<?xf32>>, !fir.shape<1>, i64)
              -> !fir.ref<f32>
        fir.store %sum to %cp : !fir.ref<f32>

        // This store is not part of the recognized kernel output.
        fir.store %side_value to %side : !fir.ref<f32>
      }

      fnacc.terminator
    } attributes {pack_targets = array<i32>}

    return
  }
}

// CHECK: error: FNACC runtime cannot lower launch:
// CHECK-SAME: unsupported operation would be discarded
// CHECK-SAME: fir.store

