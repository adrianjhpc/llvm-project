...
namespace {
  ...
  static int32_t getKernelId(fir::fngpu::LaunchOp launchOp,
    ...
    launchOp.emitWarning(
			 "fngpu.launch has no fngpu.kernel_id attribute; using walk-order fallback");
...
if (k.readArrays.size() > 3) {
  launchOp.emitError("FNGPU generic runtime supports at most three read arrays");
  signalPassFailure();
  return;
}

if (scalarCount > 3) {
  launchOp.emitError("FNGPU generic runtime supports at most three f32 scalars");
  signalPassFailure();
  return;
}
...
Value zeroF32 =
    arith::ConstantOp::create(builder, loc, f32Ty, zeroAttr);
