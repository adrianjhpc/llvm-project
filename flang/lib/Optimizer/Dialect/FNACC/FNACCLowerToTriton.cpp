...
static std::string emitExprVector(const fir::fnacc::ElementwiseKernel &k,
  ...
  case fir::fnacc::ElementwiseExprKind::SquareF: {
  ...
  case fir::fnacc::ElementwiseExprKind::And:
  case fir::fnacc::ElementwiseExprKind::Or: {
    assert(expr.operands.size() == 2 &&
    ...
    std::string result = "%pred" + std::to_string(state.nextTmp++);
    os << "  " << result << " = "
       << (expr.kind == fir::fnacc::ElementwiseExprKind::And ? "arith.andi"
                                                             : "arith.ori")
...
static void emitTritonStencil2D(const fir::fnacc::ElementwiseKernel &k,
                                int64_t blockX, int64_t blockY,
                                StringRef kernelName, llvm::raw_ostream &os) {
  assert(k.kind == fir::fnacc::ElementwiseKernelKind::Stencil2D);
  assert(!k.arrayArguments.empty() && !k.outputs.empty());
  ...
  auto emitLinearOffset =
      [&](unsigned array, ArrayRef<int64_t> offsets,
          ...
          ArrayRef<std::shared_ptr<fir::fnacc::ElementwiseIndexExpr>>
              indexExpressions,
          StringRef stem) {
